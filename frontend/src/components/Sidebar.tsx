...
export default function Sidebar({ role }: { role: string }) {
  ...
  return (
    <>
      <div className="md:hidden fixed top-0 left-0 right-0 z-40 h-14 bg-gray-950 border-b border-gray-800 flex items-center px-4 gap-3">
      ...
      <aside className={"fixed inset-y-0 left-0 z-50 w-64 bg-gray-950 border-r border-gray-800 flex flex-col transform transition-transform duration-200 ease-in-out md:static md:transform-none " + (open ? "translate-x-0" : "-translate-x-full")}>
        {navContent}
      </aside>
    </>
  );
