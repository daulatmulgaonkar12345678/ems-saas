...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    if (role !== 'society_admin') return router.push('/login');
  ...
  const getStatusStyles = (status: string) => {
    switch(status) {
      ...
      case 'FULL': return 'border-red-500/50 bg-red-500/5';
              ...
              <div key={key} className={`p-6 rounded-2xl border-2 ${getStatusStyles(wing.status)} transition-all hover:scale-[1.02]`}>
                <div className="flex justify-between items-start mb-6">
                  ...
                  <span className={`text-xs font-bold px-3 py-1 rounded-full ${
                    wing.status === 'ACTIVE' ? 'bg-green-500 text-black' : 
                    wing.status === 'FULL' ? 'bg-red-500 text-white' : 'bg-gray-700 text-gray-300'
                  }`}>
                    {wing.status}
                  </span>
                </div>
                ...
                <div className="w-full h-3 bg-gray-800 rounded-full overflow-hidden">
                  <div 
                    className={`h-full rounded-full transition-all ${
                      wing.status === 'ACTIVE' ? 'bg-green-500' : 
                      wing.status === 'FULL' ? 'bg-red-500' : 'bg-gray-600'
                    }`}
